...
<div class="ChangesByVariation" runat="server">
    ...
    */
	...
	<%-- ▽在庫切れ可否▽ --%>
	<style>/* <span visible='<%# ProductListUtility.IsProductSoldOut(this.ProductMaster) %>' runat="server" class="soldout">SOLDOUT</span> */</style>
	<%-- △在庫切れ可否△ --%>
</div>
</div>
<%-- ↑バリエーション変更時の表示更新領域を指定しています --%>
...
<HeaderTemplate>
		<div class="bl_subImage_unit">
      
</HeaderTemplate>
<ItemTemplate>
		<div visible='<%# IsSubImagesNoLimit((int)Eval(Constants.FIELD_PRODUCTSUBIMAGESETTING_PRODUCT_SUB_IMAGE_NO)) %>' runat="server">
...
/*
  <ul class="btnListContact">
    ...
    --><li><% if(Constants.VARIATION_FAVORITE_CORRESPONDENCE){ %>
      <asp:LinkButton ID="lbAddFavoriteId2" OnClick="lbAddFavorite_Click" runat="server">
      ...
      <asp:LinkButton ID="lbAddFavorite2" runat="server" OnClick="lbAddFavorite_Click">お気に入りに追加</asp:LinkButton>	<% } %>
    </li>
  </ul>
*/
</style>
...
<div class="ChangesByVariation" runat="server">
	<!-- 商品名 -->
	<h2 class="bl_productDetail_ttl el_lv2_ttl"><%# WebSanitizer.HtmlEncode(GetProductData("name")) %></h2>
  <!-- キャッチコピー -->
  <div class="bl_productDetailcatchCopy_wrapper">
	  <h3 class="bl_productDetailcatchCopy el_lv1_ttl"><%# WebSanitizer.HtmlEncode(GetProductData("catchcopy")) %></h3>
  </div>
  <!-- 商品詳細1 -->
  <div class="bl_productDetail_description1_wrapper">
    <div class="bl_productDetail_description1"><%# GetProductDataHtml("desc_detail1") %></div>
  </div>
	...
	<div visible='<%# GetProductMemberRankPriceValid(this.ProductMaster, (this.HasVariation == false) || (this.VariationSelected)) %>' runat="server">
      ...
      <span class="bl_productDetail_price_num"><strike><%#: CurrencyManager.ToPrice(ProductPage.GetProductPriceNumeric(this.ProductMaster, (this.HasVariation == false) || (this.VariationSelected))) %></strike></span>
      <span class="bl_productDetail_price_min"><%# WebSanitizer.HtmlEncode(GetTaxIncludeString(this.ProductMaster)) %></span>
      </p>
  ...
  <div visible='<%# GetProductNormalPriceValid(this.ProductMaster, (this.HasVariation == false) || (this.VariationSelected)) %>' runat="server">
    <p class="productPrice"><span class="bl_productDetail_price_label">価格</span>
      <span class="bl_productDetail_price_num"><%#: CurrencyManager.ToPrice(ProductPage.GetProductPriceNumeric(this.ProductMaster, (this.HasVariation == false) || (this.VariationSelected))) %></span>
      <span class="bl_productDetail_price_min"><%# WebSanitizer.HtmlEncode(GetTaxIncludeString(this.ProductMaster)) %></span>
    </p>
  </div>
	<%-- △商品通常価格有効△ --%>
...
<HeaderTemplate>
		<div class="variation_item_unit">
		<p class="title">バリエーション</p>
	<ul class="variationImage_unit">
</HeaderTemplate>
<ItemTemplate>
	<li class="variationImage">
...
<p class="error" style='<%# this.AlertMessage.Contains("在庫がありません") ? "display:none" : "" %>'><%# WebSanitizer.HtmlEncode(this.AlertMessage) %></p>
...
<div class="bl_productDetail_amountAndCart">
  <div class="bl_productDetail_addCard_amount" style='<%# (this.IsSelectingVariationExist == false) ? "display:none" : "" %>' runat="server" visible="<%# this.ArrivalMailKbn != Constants.FLG_USERPRODUCTARRIVALMAIL_ARRIVAL_MAIL_KBN_ARRIVAL %>">
    <div class="bl_productDetail_addCard_amount_label">数量</div>
	<div class="input_box">
...
<div visible="<%# this.ArrivalMailKbn != Constants.FLG_USERPRODUCTARRIVALMAIL_ARRIVAL_MAIL_KBN_ARRIVAL %>" runat="server" class="productCart_wrapper">

<div class="productCart">
<!-- カート投入リンク -->
<div class="addCart">
	<p class="btnCart bl_productDetail_addCart_addBtn">
	...
	<p class="btnCart bl_productDetail_addCart_addBtn teiki">
...
<asp:LinkButton ID="lbRequestArrivalMail2" Runat="server" OnCommand="ViewRegsiterArrivalMailForm_Command" CommandArgument="Arrival" class="btn btn-mid btn-inverse btn-restocking">
再入荷お知らせメール
</asp:LinkButton>
<p>
<style>
...
<div class="bl_productDetail_favBtn_area">
  <%-- <a class="bl_productDetail_favBtn">
    <img src="<%= Constants.PATH_ROOT %>Contents/ImagesPkg/new_theme/pc_btn_harticon.svg" alt="" class="bl_productDetail_favBtn_lefticon">
    この商品をお気に入りに追加する</a> --%>

	<% if(Constants.VARIATION_FAVORITE_CORRESPONDENCE){ %>
      <asp:LinkButton ID="lbAddFavoriteId" OnClick="lbAddFavorite_Click" runat="server" class="bl_productDetail_favBtn"><img src="<%= Constants.PATH_ROOT %>Contents/ImagesPkg/new_theme/pc_btn_harticon.svg" alt="" class="bl_productDetail_favBtn_lefticon"><%# (FavoriteDisplayWord((string) GetKeyValue(this.ProductMaster, Constants.FIELD_FAVORITE_SHOP_ID), this.LoginUserId, (string) GetKeyValue(this.ProductMaster, Constants.FIELD_FAVORITE_PRODUCT_ID), "")) ? "お気に入り登録済み" : "お気に入りに追加" %>
      </asp:LinkButton><% }else{ %>
      <asp:LinkButton ID="lbAddFavorite" runat="server" OnClick="lbAddFavorite_Click"><img src="<%= Constants.PATH_ROOT %>Contents/ImagesPkg/new_theme/pc_btn_harticon.svg" alt="">お気に入りに追加</asp:LinkButton>	<% } %>
  ...
  <table>
    ...
    <ItemTemplate>
      ...
      <td Visible='<%# (string)GetKeyValue(Container.DataItem, Constants.FIELD_PRODUCT_STOCK_MANAGEMENT_KBN) != "0" %>' runat="server">
        ...
        <div class="js_stock_info" visible='<%# ((string)GetKeyValue(Container.DataItem, "StockMessage") != "") %>' runat="server">
      ...
      <% if(Constants.VARIATION_FAVORITE_CORRESPONDENCE){ %>
      <p class="btnCart">
        <asp:LinkButton ID="favoriteVariation" runat="server" CommandName="AddFavorite" class="btn btn-mid js_btn_fav">
      ...
      <asp:LinkButton CommandName="SmartArrivalMail" CommandArgument="Arrival" Runat="server" class="btn btn-mid btn-inverse btn-restocking">
      入荷お知らせメール申込
      </asp:LinkButton>
      <p>
...
<div class="bl_productDetail_messageBar">8,000円以上のご注文で送料無料</div>
<style>
/*   <!-- 商品詳細2 -->
  <div id="detailTwo">
...
<div visible='<%# StringUtility.ToEmpty(GetProductData("return_exchange_message")) != "" %>' runat="server">
	...
	<div class="productSellInfo">
		...
		<%if (ShopMessage.GetMessage("ReturnSpecialContractPage") != "") {%>
			（< href="<%= WebSanitizer.HtmlEncode(Constants.PATH_ROOT) %><%= WebSanitizer.HtmlEncode(ShopMessage.GetMessage("ReturnSpecialContractPage")) %>" target="_blank" style='font-size:10px'>返品特約</a>）
		<%} %>
	</div>
</div>
</div>
...
<asp:Repeater DataSource=<%# this.ProductCrossSellList %> Visible="<%# this.ProductCrossSellList.Count != 0 %>" runat="server">
<HeaderTemplate>
<div id="dvCrossSell" class="clearFix bl_top_recommend bl_section" style="width:100%;">
<style>
  /*<p class="title">関連商品</p>*/
</style>

<div class="bl_top_recommend_head bl_section_head">
  <span class="bl_top_recommend_subTtl bl_section_subTtl hp_ff_vollkorn">RELATED ITEMS</span>
...
<ItemTemplate>
 ...
 <a href="<%# WebSanitizer.UrlAttrHtmlEncode(CreateProductDetailUrlUseProductCategory(Container.DataItem, "")) %>" class="bl_products_item">
  <figure class="bl_products_item_img">
      <w2c:ProductImage ImageTagId="picture" ImageSize="LL" ProductMaster=<%# Container.DataItem %> IsVariation="false" runat="server" />
  </figure>
...
/*   <w2c:ProductIcon IconNo="2" ProductMaster="<%# this.ProductMaster %>" runat="server" />
  ...
  <w2c:ProductIcon IconNo="10" ProductMaster="<%# this.ProductMaster %>" runat="server" /> */
    ...
    <h4 class="bl_products_item_ttl"><%# WebSanitizer.HtmlEncode(Eval(Constants.FIELD_PRODUCT_NAME)) %></h4>
    <%-- ▽在庫切れ可否▽ --%>
    <span class="bl_products_item_price bl_products_item_price__theme_soldout bl_soldoutLabel soldout" visible='<%# ProductListUtility.IsProductSoldOut(Container.DataItem) %>' runat="server">SOLDOUT</span>
    <%-- △在庫切れ可否△ --%>
    <!-- 在庫がある場合のみ価格を表示↓ -->
    <div visible="<%# !ProductListUtility.IsProductSoldOut(Container.DataItem) %>" runat="server" class="bl_products_item_price_unit">
    <%-- ▽商品会員ランク価格有効▽ --%>
  <p visible='<%# GetProductMemberRankPriceValid(Container.DataItem) %>' runat="server">
  <strike><%#: CurrencyManager.ToPrice(ProductPage.GetProductPriceNumeric(Container.DataItem)) %></strike><br />
<%#: CurrencyManager.ToPrice(ProductPage.GetProductMemberRankPrice(Container.DataItem, false)) %>
...
<%-- △商品会員ランク価格有効△ --%>
<%-- ▽商品セール価格有効▽ --%>
  <p visible='<%# ProductPage.GetProductTimeSalesValid(Container.DataItem) %>' runat="server" class="bl_products_item_price">
  <strike><%#: CurrencyManager.ToPrice(ProductPage.GetProductPriceNumeric(Container.DataItem)) %></strike><br />
<%#: CurrencyManager.ToPrice(ProductPage.GetProductTimeSalePriceNumeric(Container.DataItem)) %>
  </p>
<%-- △商品セール価格有効△ --%>
<%-- ▽商品特別価格有効▽ --%>
  <p visible='<%# ProductPage.GetProductSpecialPriceValid(Container.DataItem) %>' runat="server" class="bl_products_item_price">
  <strike><%#: CurrencyManager.ToPrice(ProductPage.GetProductPriceNumeric(Container.DataItem)) %></strike><br />
<%#: CurrencyManager.ToPrice(ProductPage.GetProductSpecialPriceNumeric(Container.DataItem)) %>
  </p>
<%-- △商品特別価格有効△ --%>
<%-- ▽商品通常価格有効▽ --%>
  <p visible='<%# ProductPage.GetProductNormalPriceValid(Container.DataItem) %>' runat="server" class="bl_products_item_price">
...
<%-- ▽定期購入有効▽ --%>
<% if (Constants.FIXEDPURCHASE_OPTION_ENABLED) {%>
  <p visible='<%# (GetKeyValue(Container.DataItem, Constants.FIELD_PRODUCT_FIXED_PURCHASE_FLG).ToString() != Constants.FLG_PRODUCT_FIXED_PURCHASE_FLG_INVALID) && (this.IsUserFixedPurchaseAble) %>' runat="server" class="bl_products_item_price">
    <span visible='<%# IsProductFixedPurchaseFirsttimePriceValid(Container.DataItem) %>' runat="server">
      <p class="bl_products_item_price">定期初回価格:<span><%#: CurrencyManager.ToPrice(ProductPage.GetProductFixedPurchaseFirsttimePrice(Container.DataItem)) %></span></p>
    </span>
    <p class="bl_products_item_price">定期通常価格:<span><%#: CurrencyManager.ToPrice(ProductPage.GetProductFixedPurchasePrice(Container.DataItem)) %></span></p>
  </p>
<% } %>
<%-- △定期購入有効△ --%>
    </div>
